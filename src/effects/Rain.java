...
public class Rain {
    ...
    private Point2D.Float getRndPos() {
        return new Point2D.Float((int) getNewX(0), random.nextInt(Game.SCREEN_HEIGHT));
    }

    public void update(int xLvlOffset) {
        for (Point2D.Float p : drops) {
            p.y += rainSpeed;
            if (p.y >= Game.SCREEN_HEIGHT) {
                p.y = -20;
    ...
    private float getNewX(int xLvlOffset) {
        float value = (-Game.SCREEN_WIDTH) + random.nextInt((int) (Game.SCREEN_WIDTH * 3f)) + xLvlOffset;
        return value;
    }
